# Unit-test standard for `yx_navigation_flutter`

This document is the source of truth for how unit tests are written in this
package. It is kept in sync with the equivalent document in the core
[`yx_navigation`](https://pub.dev/packages/yx_navigation) package — both
packages follow the same rules so that contributors can move between them
without relearning conventions.
...
3. **Unit vs integration.** `*_test.dart` files in the package `test/`
   directory are treated as solitary unit tests. Wider widget scenarios with
   a real `MaterialApp.router` / real `StateManager` / real router live in
...
21. **Coverage gate: 80%+** at the unit level. Verified locally via
...
24. **Typed matchers instead of `throwsA(isA<X>())`**: `throwsStateError`,
    `throwsArgumentError`, `throwsFormatException`, `throwsAssertionError`
    (available in this package via `package:flutter_test`), `throwsRangeError`,
...
If you are asked to write or fix a test in this package, walk the checklist
top to bottom:
